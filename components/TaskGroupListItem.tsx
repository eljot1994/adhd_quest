...
interface TaskGroupListItemProps {
  ...
  userLevel: number;
  onCompleteSubTask: (groupId: string, subTaskId: string, difficulty: TaskDifficulty) => void;
      ...
      <div className="space-y-2">
        {group.subTasks.map(subTask => {
          if (editingSubTaskId === subTask.id) {
            return (
              <div key={subTask.id} className="bg-slate-200 dark:bg-slate-700 p-2 rounded-lg flex items-center justify-between gap-2 ring-1 ring-cyan-500">
                <input
                  type="text"
                ...
                <div className="flex items-center space-x-1">
                  <button onClick={handleSaveEdit} className="p-1 text-green-600 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-500/20 rounded-full transition-colors" title="Zapisz">
                    <CheckIcon className="w-4 h-4" />
                  </button>
                  <button onClick={handleCancelEdit} className="p-1 text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-500/20 rounded-full transition-colors" title="Anuluj">
          ...
          return (
            <div key={subTask.id} className={`flex items-center justify-between p-2 rounded-md transition-colors ${subTask.completed ? 'bg-slate-100 dark:bg-slate-700/50' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <span className={`font-medium truncate ${subTask.completed ? 'line-through text-slate-500 dark:text-slate-400' : 'text-slate-800 dark:text-slate-100'}`}>
                  {subTask.name}
                </span>
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border whitespace-nowrap ${difficultyColors[subTask.difficulty]} ${subTask.completed ? 'opacity-60' : ''}`}>
                  {subTask.difficulty}
                </span>
              </div>
              <div className="flex items-center space-x-1 flex-shrink-0">
                {!subTask.completed && (
                  <>
                    <button
                      onClick={() => onCompleteSubTask(group.id, subTask.id, subTask.difficulty)}
