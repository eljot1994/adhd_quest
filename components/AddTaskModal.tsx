...
import { predefinedTaskGroups, TaskGroup } from "./predefinedTaskGroups";
...
const AddTaskModal: React.FC<AddTaskModalProps> = ({
  ...
  const [newTaskDifficulty, setNewTaskDifficulty] = useState<TaskDifficulty>(
    TaskDifficulty.Easy
  );
      ...
      ...(isRecurring &&
        canCreateRecurringTasks && {
          ...
          ...(recurringInterval === "custom" && {
            customRecurringDays: Math.max(1, parseInt(customDays, 10) || 1),
          }),
        }),
    };
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 dark:bg-black/70 flex items-end justify-center z-50 transition-opacity"
      onClick={onClose}
    >
      <div
                  ...
                  {deadlineType === DeadlineType.Duration && (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="Godz."
                        min="0"
                        className="w-full bg-slate-100 dark:bg-slate-700 rounded-md px-3 py-2 border border-slate-300 dark:border-slate-600 focus:ring-cyan-500"
                      />
                      <input
                        type="number"
                ...
                <div>
                  {canSelectDifficulty ? (
                    <select
                      ...
                      onChange={(e) =>
                        setNewTaskDifficulty(e.target.value as TaskDifficulty)
                      }
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Wybór trudności od poz. {UNLOCK_LEVELS.SELECT_DIFFICULTY}.
                    </p>
                  )}
                </div>
                <button
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 text-left transition-all flex items-center gap-4 ${
                      isUnlocked
                        ? "border-cyan-500/50 bg-cyan-50 dark:bg-cyan-500/10 hover:bg-cyan-100 dark:hover:bg-cyan-500/20 cursor-pointer"
                        : "border-slate-300/70 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 cursor-not-allowed"
                    }`}
                  >
                    {isUnlocked ? (
                    ...
                    <div>
                      ...
                      {!isUnlocked && (
                        <p className="text-xs">
                          Odblokuj na poziomie {group.unlockLevel}
                        </p>
                      )}
                    </div>
                  </button>
